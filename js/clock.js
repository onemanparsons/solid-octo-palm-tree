...
// This function sets the clock to the current time
function setTime() {
    // Get the current date and time
    var now = new Date();

    // Calculate the degrees to rotate for each hand
    var secondsDegrees = (now.getSeconds() / 60) * 360;
    var minutesDegrees = ((now.getMinutes() + now.getSeconds() / 60) / 60) * 360;

    // Rotate the clock hands
    secondsHand.setAttribute('transform', 'rotate(' + secondsDegrees + ')');
    minutesHand.setAttribute('transform', 'rotate(' + minutesDegrees + ')');
}

// Call setTime every second
setInterval(setTime, 1000);

// Call setTime once at the start to set the initial time
setTime(); */